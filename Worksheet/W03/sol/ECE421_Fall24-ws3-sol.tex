...
%	Initialize $\kappa$-means with the centroids $\mu_1 = 2$ and $\mu_2 = 4$. You may assume that if a point $x^{(i)}$ is equally distant to multiple centroids $\mu_k$, the point will be assigned to the centroid with the smallest index, i.e., the smallest $k \in \underset{k}{\arg\min}\left(\|x^{(i)} - \mu_k\|^2\right)$.
	...
	\end{equation*} 
		...
		\begin{question}
			...
			\begin{answer}
				It suffices to prove that $\underline{z}^\top X^\top X \underline{z} \geq 0$ for any non-zero vector $\underline{z}$. Observe that $$\underline{z}^\top X^\top X \underline{z} = \|X \underline{z}\|_2^2 \geq 0.$$
			\end{answer}
			\item \label{qq}Demonstrate that matrix $X^\top X + \lambda N  I$, where $I$ is the identity matrix, is invertible by proving that none of its eigenvalues are zero.\\\hint{Assume $\underline{v}$ is an eigenvector of $X^\top X$ with corresponding eigenvalue $\mu$. In~\ref{q}, you proved that $\mu \geq 0$. Now, show that $\underline{v}$ is also an eigenvector of $X^\top X + \lambda N  I$ and find its corresponding eigenvalue.}
			\begin{answer}
