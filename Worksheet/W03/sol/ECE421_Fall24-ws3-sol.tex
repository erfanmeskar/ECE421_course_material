...
\begin{question}[start=0]
	...
	You are given a dataset $\mathcal{D} = \{(\underline{x}_n, y_n)\}_{n=1}^N$, where $\underline{x}_n \in \mathbb{R}^{d+1}, d \geq 1$, and $y_n \in \mathbb{R}$. We would like to train a regularized linear regression model, where the mean squared loss is augmented with an $\ell_2$ regularization penalty $\|\underline{w}\|_2^2$ on the weight parameter $\underline{w}  \in \mathbb{R}^{d+1}$, \textit{i.e.},
	\begin{equation*}
